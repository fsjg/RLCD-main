...
def divide_data():
    ...
    :return:
    '''
    with open('RLCD-main/data/ASSIST17/log_data.json', encoding='utf8') as i_f:
        stus = json.load(i_f)
    # 1. delete students who have fewer than min_log response logs
    stu_i = 0
    l_log = 0
    ...
    with open('RLCD-main/data/ASSIST17/train_set.json', 'w', encoding='utf8') as output_file:
        json.dump(train_set, output_file, indent=4, ensure_ascii=False)
    with open('RLCD-main/data/ASSIST17/test_set.json', 'w', encoding='utf8') as output_file:
